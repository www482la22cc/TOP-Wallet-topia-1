...
function processGetAddrResponse(response: any) {
  ...
  const addrString = Buffer.from(
    partialResponse.slice(0, addrStringLength)
  ).toString()
...
export default class FilecoinApp {
  ...
  async getAddressAndPubKey(path: any) {
  ...
  async showAddressAndPubKey(path: any) {
    return this.serializePath(path)
      .then((data) => {
        return this.transport
          .send(
  ...
  async signSendChunk(chunkIdx: any, chunkNum: any, chunk: any, ins: any) {
    switch (this.versionResponse.major) {
      case 0:
      case 1:
  ...
  async signGeneric(path: any, message: any, ins: any) {
    return this.signGetChunks(path, message).then((chunks) => {
      return this.signSendChunk(1, chunks.length, chunks[0], ins).then(
        async (response) => {
          let result = {
